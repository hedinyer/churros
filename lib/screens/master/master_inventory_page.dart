...
class _MasterInventoryPageState extends State<MasterInventoryPage>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final inventarioActual =
          await SupabaseService.getInventarioActual(widget.sucursal.id);

      setState(() {
        _productos = productos;
        _categoriasMap = categoriasMap;
        _inventarioActual = inventarioActual;
        _inventarioEditado = Map.from(inventarioActual);
        _isLoading = false;
      });
  ...
  bool _hasChanges() {
    if (_inventarioEditado.length != _inventarioActual.length) return true;
    for (var entry in _inventarioEditado.entries) {
      if (_inventarioActual[entry.key] != entry.value) return true;
    }
    return false;
  }
  ...
  int _changedCount() {
    int count = 0;
    for (var entry in _inventarioEditado.entries) {
      if (_inventarioActual[entry.key] != entry.value) count++;
    }
    return count;
  }
  ...
  Future<void> _saveInventory() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => _buildConfirmDialog(
        ...
        message:
            '¿Actualizar el inventario de ${widget.sucursal.nombre}? (${_changedCount()} productos modificados)',
        confirmText: 'Guardar',
    ...
    try {
      final success = await SupabaseService.actualizarInventarioActual(
        sucursalId: widget.sucursal.id,
        inventario: _inventarioEditado,
      );
      ...
      if (success) {
        setState(() => _inventarioActual = Map.from(_inventarioEditado));
        ...
        _showErrorSnackbar('Error al actualizar el inventario');
      }
    } catch (e) {
      if (!mounted) return;
      Navigator.pop(context);
  ...
  Future<void> _resetAllInventory() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => _buildConfirmDialog(
        title: 'Poner todo en 0',
        message:
            '¿Poner TODO el inventario de ${widget.sucursal.nombre} en 0?\n\nRecuerda guardar después.',
    ...
    setState(() {
      _inventarioEditado = {for (var p in _productos) p.id: 0};
    });
    _checkAndAnimateSaveBar();
      ...
      SnackBar(
        content: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Text('Todo en 0. Guarda para aplicar.'),
          ],
        ),
  ...
  Widget _buildProductCard(
    ...
    final originalQty = _inventarioActual[producto.id] ?? 0;
                ...
                children: [
                  _buildInlineQtyBtn(
                    ...
                    onTap: () {
                      if (cantidad > 0) {
                        setState(() {
                          _inventarioEditado[producto.id] = cantidad - 1;
                        });
                        _checkAndAnimateSaveBar();
                      }
                    },
